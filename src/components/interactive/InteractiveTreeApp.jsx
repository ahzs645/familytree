...
export function InteractiveTreeApp() {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {showPeople && (
        <div className="shrink-0" style={{ width: isMobile ? '100%' : 'min(280px, 46vw)' }}>
      ...
      {showFocus && (
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          {showHeader && (
          <div className="flex min-h-16 shrink-0 flex-wrap items-center justify-between gap-3.5 border-b border-border bg-card px-4 py-2.5">
            {isMobile && (
              <Button
                variant="outline"
                size="icon"
              ...
              <div className="flex max-w-full shrink-0 items-center gap-1 overflow-x-auto rounded-md border border-border bg-secondary p-1" role="tablist" aria-label="Tree view mode">
                {TREE_VIEW_MODES.map(([id, label]) => (
                  <button
                    ...
                    className={cn(
                      'min-w-14 shrink-0 cursor-pointer rounded-md border border-transparent px-2.5 py-1.5 text-xs font-bold',
                      viewMode === id
                        ? 'bg-background text-foreground shadow-[0_1px_6px_rgb(0_0_0/0.08)]'
...
function TreeInspector({
  ...
  return (
    <aside className={inspectorClass}>
      <div className="flex items-center gap-3 px-1 pb-3.5 pt-2">
        <div
          className="grid h-[54px] w-[54px] shrink-0 place-items-center rounded-full text-[17px] font-extrabold text-white shadow-[inset_0_0_0_2px_rgb(255_255_255/0.58),0_8px_18px_rgb(0_0_0/0.12)]"
          style={{ background: avatarFill(self.gender) }}
          aria-hidden="true"
        >
          {initials(self.fullName)}
        </div>
        <div className="min-w-0">
          <div className="truncate text-base font-bold text-foreground" title={self.fullName}><BdiText>{self.fullName}</BdiText></div>
          <div className="mt-0.5 text-xs text-muted-foreground"><LtrText>{lifeSpanLabel(self) || 'No life dates'}</LtrText></div>
        </div>
      </div>

      <InspectorSection title="Actions">
        <div className="grid grid-cols-2 gap-2">
          <Button className={tileButtonClass} onClick={() => onEditPerson(self.recordName)}>Edit Person</Button>
          <Button className={tileButtonClass} onClick={() => onOpenAncestorChart(self.recordName)}>Ancestors</Button>
          <Button className={tileButtonClass} onClick={() => onOpenDescendantChart(self.recordName)}>Descendants</Button>
          {context.families[0]?.family?.recordName ? (
            <Button className={tileButtonClass} onClick={() => onOpenFamily(context.families[0].family.recordName)}>Select Family</Button>
          ) : (
            <Button className={tileButtonClass} disabled>Select Family</Button>
          )}
        </div>
      </InspectorSection>
      ...
      <InspectorSection title={`Events (${context.events.length})`}>
        <div className="grid gap-2">
          {context.events.slice(0, 4).map((event) => (
            <div key={event.recordName} className="flex items-start gap-2">
          ...
          {context.events.length === 0 && <div className="text-xs text-muted-foreground">No events recorded.</div>}
        </div>
      </InspectorSection>
    </aside>
  );
...
const inspectorClass = 'w-[318px] shrink-0 overflow-auto border-s border-border bg-card p-3.5';
const tileButtonClass = 'min-h-[58px] font-bold';
...
function InspectorSection({ title, children }) {
  return (
    <section className="mb-3 rounded-md border border-border bg-background p-2.5">
      <div className="mb-2.5 text-xs font-extrabold text-foreground">{title}</div>
      {children}
    </section>
  );
}
...
function RelationGroup({ title, people, onPick }) {
  return (
    <div className="mb-3">
      <div className="mb-1.5 text-xs font-extrabold uppercase text-muted-foreground">{title}</div>
      {people.length === 0 ? (
        ...
        <div className="grid gap-1.5">
          {people.slice(0, 5).map((person) => (
            <button
              ...
              type="button"
              className="w-full cursor-pointer rounded-md border border-border bg-card px-2.5 py-2 text-start"
              onClick={() => onPick(person.recordName)}
            >
              <span className="block truncate text-xs font-bold text-foreground"><BdiText>{person.fullName}</BdiText></span>
              <span className="mt-0.5 block text-xs text-muted-foreground"><LtrText>{lifeSpanLabel(person)}</LtrText></span>
            </button>
          ))}
        </div>
      )}
...
// The avatar fills are fixed light gradients (not theme tokens), so the white
// initials text stays readable in both themes.
function avatarFill(gender) {
