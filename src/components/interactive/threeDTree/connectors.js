...
export function makeConnector(link, nodes, palette) {
  ...
  const tubeRadius = link.emphasis ? 4.0 : type === 'partner' ? 2.4 : 3.0;
  for (let i = 1; i < points.length; i += 1) {
    group.add(makeRibbonSegment(points[i - 1], points[i], palette.shadow, tubeRadius + 3.4, 0.12, -7, 3));
  }
  for (let i = 1; i < points.length; i += 1) {
  ...
  if (type === 'family' || link.emphasis) {
    for (const point of uniqueConnectorPoints(points)) {
      group.add(makeConnectionCap(point, color, link.emphasis ? 8 : 6));
    }
  }
  return group;
}
...
function makeConnectorTexture(color, opacity) {
  return makeCanvasTexture(256, 64, (ctx, w, h) => {
    ...
    if (opacity > 0.2) {
      ...
      highlight.addColorStop(0.52, 'rgba(255,255,255,0.05)');
