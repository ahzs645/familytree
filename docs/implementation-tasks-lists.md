...
- [ ] Add dedicated `Persons` list route: `src/routes/Persons.jsx`.
...
- [ ] Add `src/routes/MarriageList.jsx` using `buildMarriageListReport` data shape with row-level partner links.
- [ ] Add `src/routes/FactsList.jsx` using `buildFactsListReport` and filter by fact type/date.
- [ ] Add `src/routes/AnniversaryList.jsx` with month/day filtering and year formatting.
- [ ] Add `src/routes/PlausibilityList.jsx` as a dedicated list entry that wraps current plausibility checker output.
...
- [ ] Add `src/routes/Lists.jsx` as a hub with counts:
  - Persons, Places, Sources, Events, Media, To-Dos, Changes, Plausibility, Anniversary, Facts, Marriage.
- [ ] Add list route cards on `/home` for fast entry.

Acceptance:
- Users can see list coverage and jump from one click.

### 3) Implement LDS-oriented and analysis lists where data exists
- [ ] Add `Distinctive Persons` list placeholder with a rule-based implementation.
  - If no `distinctive` marker exists, provide a filter-by-criteria interface (manual rules).
- [ ] Add `Person Analysis` route with reusable metrics (age, missing dates, orphaned relationships, duplicate risks).
- [ ] Evaluate and gate LDS-related lists (`LDS Ordinances`) based on actual schema presence.
...
- [ ] Convert selected report list builders into dedicated routes with `MasterDetailList` style filters:
  - Current `events`, `places`, `sources`, `to-do`, `changes` routes are used as reference UX patterns.
- [ ] Add column/label parity and sortable headers across all list routes.
